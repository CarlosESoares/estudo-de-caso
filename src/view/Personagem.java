...
import controller.ArmasDAO;
import model.Armas;
import model.Classe;
import model.Infos;
import model.Inventario;
import model.Raca;
import controller.InventarioDAO;
	...
	public static void main(String[] args) {
		...
		if (escolha == 1 ) {
			...
			for (int i = 0; i < pericias.size() - 1; i++) {
				...
				if (op == 0) {
				while ( limiteObj < 8) {
					System.out.println("o que deseja adicionar é: um objeto comum (1), uma arma (2) ou você não deseja adiconar nada?");
					int resp = in.nextInt();
					switch (resp) {
					case 1: {
						System.out.println("qual o nome do seu objeto?");
						String nomeObjeto = in.next();
						System.out.println("qual a descrição desse objeto?");
						String descObjeto = in.next();
						invent.InventarioAdd(new Inventario(nomeObjeto, descObjeto));
						limiteObj += 1;
						break;
					} 
					case 2: {
						System.out.println("qual o nome da sua Arma?");
						String nomeArma = in.next();
						System.out.println("qual a descrição da sua Arma?");
						String descArmas = in.next();
						System.out.println("qual o dano da sua Arma?");
						int danoArma = in.nextInt();
						System.out.println("qual o peso da sua Arma?");
						int pesoArma = in.nextInt();
						armas.ArmasAdd(new Armas(nomeArma, descArmas, danoArma, pesoArma));
						limiteObj += 1;  
						break;
					}
					case 3: {
				...
				for (Armas a : armas.armmas) {
					System.out.println(a.getObjeto());
					System.out.println(a.getDescObjeto());
					System.out.println(a.getDano());
					System.out.println(a.getPeso());
				}
				for (Inventario i : invent.objetos) {
					System.out.println(i.getObjeto());
					System.out.println(i.getDescObjeto());
				}
				 ...
				 if (modifica == 0) {
					 ...
					 System.out.println("você deseja excluir (0), adicionar (1), sair (2)");
					 int escol = in.nextInt();
					 switch (escol) {
					 case 0: {
						 System.out.println("o objeto que quer deletar é uma arma(0) ou um objeto comum (1)?");
						 int aORo = in.nextInt();
						 System.out.println("qual objeto você deseja remover? (o numero de sua posição)");
						 int exl = in.nextInt();
						 if (aORo == 0) {
							armas.armmas.remove(exl);
						} else if (aORo == 1 ) {
							invent.objetos.remove(exl);
						}
						
					 } 
					 case 1: {
						 if (armas.armmas.size() + invent.objetos.size() < 8) {
							 while ( limiteObj < 8) {
									System.out.println("o que deseja adicionar é: um objeto comum (1), uma arma (2) ou você não deseja adiconar nada?");
									int resp = in.nextInt();
									switch (resp) {
									...
									case 3: {
										...
										System.out.println("seu inventario está lotado!");} }
									}
